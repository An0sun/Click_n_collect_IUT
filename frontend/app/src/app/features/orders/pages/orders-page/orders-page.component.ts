...
private attachSse(orderId: number) {
  ...
  const es = new EventSource(
    new URL(`orders/sse/${orderId}`, this.baseUrl).toString()
  );
...
private loadPage(page: number) {
  this.ordersService.getOrders(page).subscribe(res => {
    ...
    this.orders = this.isAdmin ? base.filter(o => o.status !== 'CONSOMEE') : base;
...
goPrev() { if (this.currentPage > 1) this.loadPage(this.currentPage - 1); }
goNext() { if (this.currentPage < this.totalPages) this.loadPage(this.currentPage + 1); }
